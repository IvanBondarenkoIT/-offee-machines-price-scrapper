...
class WooCommerceClient:
    ...
    def get_stock_data(self) -> pd.DataFrame:
        ...
        logger.info(f"In stock (stock_status='instock'): {self.stats['in_stock_products']}")
    ...
    def _convert_to_dataframe(self, products: List[Dict]) -> pd.DataFrame:
        ...
        Filters: only products with stock_status == 'instock'
        (Note: stock_quantity can be 0 or null even for instock products)
        ...
        for product in products:
            try:
                ...
                # Filter: only products with stock_status == 'instock'
                # Note: stock_quantity can be 0 or null even for instock products
                if stock_status != 'instock':
                    continue  # Skip products that are not in stock
                
                # Parse stock_quantity (can be None or 0 for instock products)
