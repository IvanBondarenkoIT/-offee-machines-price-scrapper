...
1. Загрузка инвентаря (остатки.xls или API) - 47 товаров
2. Загрузка данных со всех сайтов - 183 товара
3. **Загрузка остатков из WooCommerce** (если включено) - товары с stock > 0
4. Извлечение моделей - 213+ товаров
5. Нормализация и сопоставление
6. Fuzzy matching для вариантов
7. Создание таблицы сравнения с остатками и ценами WooCommerce

**Результат**: `price_comparison_YYYYMMDD_HHMMSS.xlsx`

**Новые столбцы в таблице:**
- `DIM_KAVA_STOCK` - остаток на складе из WooCommerce
- `DIM_KAVA_WC_PRICE` - цена из WooCommerce (с учетом скидок)

**Настройка WooCommerce:**
- Включить в `.env`: `USE_WOOCOMMERCE_STOCK=true`
- Указать credentials: `WC_URL`, `WC_CONSUMER_KEY`, `WC_CONSUMER_SECRET`
- Опционально: `WC_USER_AGENT` для обхода Imunify360
