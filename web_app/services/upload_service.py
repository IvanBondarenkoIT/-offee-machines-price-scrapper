...
def _parse_comparison_sheet(df, upload_id):
    ...
    - Our Price
    - Quantity
    - DIM_KAVA_STOCK (optional) - WooCommerce stock quantity
    - DIM_KAVA_WC_PRICE (optional) - WooCommerce price (format: "123.45" or "150.00 \\ 120.00")
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # Process WooCommerce stock and price (if available)
            dimkava_wc_stock = None
            dimkava_wc_price = None
            
            if 'DIM_KAVA_STOCK' in df.columns:
                stock_value = row.get('DIM_KAVA_STOCK')
                if not pd.isna(stock_value) and stock_value != '-':
                    try:
                        dimkava_wc_stock = int(float(str(stock_value).strip()))
                    except (ValueError, TypeError):
                        dimkava_wc_stock = None
            
            if 'DIM_KAVA_WC_PRICE' in df.columns:
                price_value = row.get('DIM_KAVA_WC_PRICE')
                if not pd.isna(price_value) and price_value != '-':
                    dimkava_wc_price = str(price_value).strip()
            ...
            product = Product(
                ...
                our_price=our_price,
                brand=brand,
                dimkava_wc_stock=dimkava_wc_stock  # WooCommerce stock quantity
            )
            ...
            # Process DIM_KAVA_WC_PRICE as a special competitor price
            if dimkava_wc_price:
                price_str = dimkava_wc_price
                if '\\' in price_str:
                    # Has discount format: "regular \\ discount"
                ...
                # Create competitor price for WooCommerce
                wc_cp = CompetitorPrice(
                    product_id=product.id,
                    competitor='DIM_KAVA_WC',  # Special identifier for WooCommerce price
                    price=price,
