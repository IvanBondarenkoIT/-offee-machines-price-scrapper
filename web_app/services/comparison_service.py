...
def _get_comparison_data(upload):
    ...
    # Exclude DIM_KAVA_WC from regular competitors (it has its own columns)
    competitor_names = [c for c in competitor_names if c != 'DIM_KAVA_WC']
...
def export_to_excel(upload_id):
    ...
    for product in products:
        ...
        # Add WooCommerce stock and price
        if product.dimkava_wc_stock is not None:
            row['DIM_KAVA_STOCK'] = int(product.dimkava_wc_stock)
        else:
            row['DIM_KAVA_STOCK'] = '-'
        ...
        # Get WooCommerce price from competitor_prices
        wc_cp = next((cp for cp in competitor_prices if cp.competitor == 'DIM_KAVA_WC'), None)
        if wc_cp:
            if wc_cp.has_discount and wc_cp.regular_price and wc_cp.discount_price:
                row['DIM_KAVA_WC_PRICE'] = f"{float(wc_cp.regular_price):.2f} \\ {float(wc_cp.discount_price):.2f}"
            elif wc_cp.price:
                row['DIM_KAVA_WC_PRICE'] = f"{float(wc_cp.price):.2f}"
            else:
